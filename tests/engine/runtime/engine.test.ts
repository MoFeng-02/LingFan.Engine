...
describe("NVL × 历史（historyView 暴露 nvl 标记与累积行快照）", () => {
  it("NVL 段的检查点带 nvl=true 与累积行；nvl clear 分段；退出后复位", () => {
    const { engine, dispose } = makeEngine([
      { op: "say", text: "开场" },
      { op: "nvl" },
      { op: "say", text: "甲" },
    ...
    engine.start(); // 开场显示（检查点待提交）
    engine.advance(); // 提交开场 → 甲显示（NVL 段 1）
    engine.advance(); // 提交甲 → 乙显示（累积 [甲,乙]）
    engine.advance(); // 提交乙 → 丙显示（clear 后新段 [丙]）
    let view = engine.historyView(); // 已入档：开场/甲/乙
    expect(view.map((h) => h.nvl)).toEqual([false, true, true]);
    expect(view[1]!.nvlLines).toEqual(["甲"]);
    expect(view[2]!.nvlLines).toEqual(["甲", "乙"]); // 段 1 的整块画面
    engine.advance(); // 提交乙 → 尾声显示（exit 后非 NVL）
    view = engine.historyView();
    expect(view.map((h) => h.nvl)).toEqual([false, true, true, true]);
    expect(view[3]!.nvl).toBe(true);
    expect(view[3]!.nvlLines).toEqual(["丙"]); // 收缩 = clear 分段
    engine.advance(); // 提交尾声（列播完）
    view = engine.historyView();
    expect(view.at(-1)).toMatchObject({ nvl: false, nvlLines: [] });
    dispose();
  });
});
...
describe("08 §六.5 视频族（video/cutscene/seek/pause/resume/stop/skipable）", () => {
  ...
  it("video 非阻塞：发 play 命令且故事继续推进到下一等待", () => {
    const { engine, changes, dispose } = makeEngine([
      { op: "video", resource: "Video/m1.mp4", volume: 0.7 },
      { op: "say", text: "边看边讲" },
    ]);
    engine.start();
    expect(videoOf(engine)).toMatchObject({
      kind: "play",
    ...
    expect(engine.get(SYS.waiting)).toBe("dialog"); // 非阻塞：故事继续
    expect(changes.some((c) => c.key === SYS.video)).toBe(true);
    dispose();
  });

  it("cutscene 阻塞：进入 video 等待 + 建立时检查点；videoFinished 解除并继续", () => {
    const { engine, dispose } = makeEngine([
      { op: "cutscene", resource: "Video/m1.mp4", volume: 0.8, skipable: true },
      { op: "say", text: "过场后" },
    ]);
    engine.start();
  ...
  it("不可跳过 → advance fail-closed；可跳过 → 发 stop 并解除等待", () => {
    const a = makeEngine([
      { op: "cutscene", resource: "v.mp4", skipable: false },
      { op: "say", text: "x" },
    ]);
    a.engine.start();
    a.engine.advance();
    expect(
      a.errors.some(
        (e) =>
          ...
          e.payload.code === "advance-invalid",
      ),
    ).toBe(true);
    a.dispose();

    const b = makeEngine([
      { op: "cutscene", resource: "v.mp4", skipable: true },
      { op: "say", text: "x" },
    ]);
    b.engine.start();
    b.engine.advance(); // 跳过：stop 命令 + 解除等待
    expect(videoOf(b.engine)).toMatchObject({ kind: "stop" });
    expect(b.engine.get(SYS.waiting)).toBe("dialog");
    b.dispose();
  });

  it("seek/pause/resume/stop 依序发命令（seq 单调）", () => {
    const { engine, changes, dispose } = makeEngine([
      { op: "video", resource: "v.mp4" },
      { op: "seek_video", seconds: 10 },
      { op: "pause_video" },
      { op: "resume_video" },
      { op: "stop_video" },
      { op: "say", text: "x" },
    ...
    engine.start();
    const cmds = changes
      .filter((c) => c.key === SYS.video)
      .map((c) => c.value as VideoCommand);
    ...
    expect(cmds.map((c) => c.seq)).toEqual([1, 2, 3, 4, 5]);
    dispose();
  });

  it("video_skipable 持久开关作为后续缺省（显式参数优先）", () => {
    const { engine, dispose } = makeEngine([
      { op: "video_skipable", value: false },
      { op: "cutscene", resource: "v.mp4" },
      { op: "say", text: "x" },
    ]);
    engine.start();
    expect(videoOf(engine)).toMatchObject({ cutscene: true, skipable: false });
    dispose();
  });

  it("对抗性：非法 resource/seconds、未知字段 → fail-closed", () => {
    const cases: Array<{ cmd: object; code: string }> = [
      { cmd: { op: "video", resource: "" }, code: "video-invalid-resource" },
      {
        cmd: { op: "cutscene", resource: 9 },
        code: "cutscene-invalid-resource",
      },
      {
        cmd: { op: "video", resource: "a", volume: "大" },
        code: "video-invalid-volume",
      },
      {
        cmd: { op: "seek_video", seconds: -1 },
        code: "seek_video-invalid-seconds",
      },
      { cmd: { op: "seek_video" }, code: "seek_video-invalid-seconds" },
      {
        cmd: { op: "video", resource: "a", 音量: 1 },
        code: "video-unknown-field",
      },
      {
        cmd: { op: "video_skipable", value: true, extra: 1 },
        code: "video_skipable-unknown-field",
      },
    ];
    for (const { cmd, code } of cases) {
      const { engine, errors, dispose } = makeEngine([cmd]);
      engine.start();
      expect(
        errors.some(
          (e) =>
            e.payload.kind === "engine.error" &&
            (e.payload as { code?: string }).code === code,
        ),
      ).toBe(true);
      dispose();
    }
